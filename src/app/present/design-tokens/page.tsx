  ...
  const htmlContent = `
    <style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .presentation {
        ...
        color: white; position: fixed; top: 0; left: 0; z-index: 9999;
        overflow: hidden;
      }
    ...
    <div className="presentation">
      ${slides
        .map(
          (slide, index) => `
        <div className="slide ${index === 0 ? 'active title' : ''}" data-slide="${index}">
          <div className="slide-content">${parseSlide(slide)}</div>
        </div>
      `
        )
        .join('')}
      ...
      <div className="controls">
        <div className="slide-counter"><span id="current">1</span> of ${slides.length}</div>
        <div className="dots">
          ${slides.map((_, i) => `<button className="dot ${i === 0 ? 'active' : ''}" onClick={() => goTo(${i})}></button>`).join('')}
        </div>
        <div className="nav-buttons">
          <button id="prev" onClick={() => prev()} disabled>Previous</button>
          <button id="next" onClick={() => next()} className="primary">Next</button>
          <button onClick={() => document.documentElement.requestFullscreen()}>Fullscreen (F)</button>
        </div>
      </div>
      <button className="exit-btn" onClick={() => window.location.href='/demo/presentations'}>Exit</button>
    </div>
    ...
    <script dangerouslySetInnerHTML={{__html: \`
      ...
      document.addEventListener('keydown', e => {
        switch(e.key) {
          ...
          case 'f': case 'F': e.preventDefault(); document.documentElement.requestFullscreen(); break;
          case 'Escape': e.preventDefault(); window.location.href='/demo/presentations'; break;
          default:
